...
input='sub-4049_ses-102521NORDIC_5_minutes_of_data_at_FD_0.3.dconn_CorrTo_sub-4049_ses-102521NORDIC_all_frames_at_FD_0.3.dconnall_vertices.txt';
rel_val=readtable(input);

example_dscalar.cdata=table2array(rel_val);

cifti_write(example_dscalar, 'rel_map_5min_NORDIC.dscalar.nii');
...
pics_folder = '/panfs/roc/groups/4/miran045/shared/projects/WashU_Nordic/reliability_maps';
scalar_name=['rel_map_5min_NORDIC.dscalar.nii'];
output_name=['rel_map_5min_NORDIC_2'];
cmd = [pics_code_path ' ' scalar_name ' ' output_name '_fig ' pics_folder ' FALSE 0 0.8 JET256 FALSE 0 20 THRESHOLD_TEST_SHOW_OUTSIDE TRUE  ' make_subcortical_images ' png 8 118 FALSE ' settings.path{4} ' ' settings.path{2} ' ' settings.path{3}];
disp(cmd);
system(cmd);